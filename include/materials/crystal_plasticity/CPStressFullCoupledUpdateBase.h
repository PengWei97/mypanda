...
class CPStressFullCoupledUpdateBase : public Material
{
public:
  static InputParameters validParams();

  CPStressFullCoupledUpdateBase(const InputParameters & parameters);

  /// Sets the value of the global variable _qp for inheriting classes
  void setQp(const unsigned int & qp);

  /// Sets the value of the global variable _active_op_index for inheriting classes
  void setActiveOpIndex(const unsigned int & active_op_index);
  ...
  virtual void resetQpStatefulProperties(const unsigned int & op_index);
  ...
  // this virtual method is called to reset the constitutive internal state variables
  virtual void resetInitialConstitutiveVariableValues() {}
  ...
  // this virtual method is called to calculate the plastic energy for each active grains
  virtual void calculatePlasticityEnergy(Real & plastic_energy_grX, const unsigned int & op_index);
protected:
  ...
  /// the heat dissipationin coefficient in plasticity deformation
  const Real _coeff_heat_dissip;
  ...
  /// Number of iterations for the internal variable update
  const unsigned int _op_num;
  const std::vector<const VariableValue *> _vals;
  Real _active_op_index;

  ///@{Slip system resistance
  std::vector<MaterialProperty<std::vector<Real>> *> _slip_resistance_gr;
  std::vector<const MaterialProperty<std::vector<Real>> *> _slip_resistance_gr_old;
  ///@}

  ///@{Slip system shear strain
  std::vector<MaterialProperty<std::vector<Real>> *> _slip_shear_strain_gr;
  std::vector<const MaterialProperty<std::vector<Real>> *> _slip_shear_strain_gr_old;
  ///@}
