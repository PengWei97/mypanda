...
registerMooseObject("mypandaApp", FeatureDataVectorPostprocessor);
...
void
FeatureDataVectorPostprocessor::accumulateVolumes(
    const Elem * elem,
...
Real
FeatureDataVectorPostprocessor::computeIntegral(std::size_t var_index) const
{
  Real sum = 0;
...
void
FeatureDataVectorPostprocessor::accumulateBoundaryFaces(
    const Elem * elem,
    const std::vector<unsigned int> & var_to_features,
    std::size_t libmesh_dbg_var(num_features),
    unsigned int side)
...
Real
FeatureDataVectorPostprocessor::computeFaceIntegral(std::size_t var_index) const
{
  Real sum = 0;
