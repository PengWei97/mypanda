...
#include "ComputeMultiCPStress.h"
...
registerMooseObject("mypandaApp", ComputeMultiCPStress);

InputParameters
ComputeMultiCPStress::validParams()
{
...
ComputeMultiCPStress::ComputeMultiCPStress(
...
void
ComputeMultiCPStress::initQpStatefulProperties()
{
...
void
ComputeMultiCPStress::initialSetup()
{
  ...
  for (unsigned int i = 0; i < _num_models; ++i)
  {
    ...
    if (model)
    {
      ...
      mooseError("Model " + model_names[i] +
                 " is not compatible with ComputeMultiCPStress");
  }
...
void
ComputeMultiCPStress::computeQpStress()
{
  for (unsigned int i = 0; i < _num_models; ++i)
  {
    _models[i]->setQp(_qp);
...
void
ComputeMultiCPStress::updateStress(RankTwoTensor & cauchy_stress,
                                                     RankFourTensor & jacobian_mult)
{
  ...
  do
  {
    ...
    if (substep_iter > _max_substep_iter && _convergence_failed)
      mooseException("ComputeMultiCPStress: Constitutive failure");
...
void
ComputeMultiCPStress::preSolveQp()
{
  for (unsigned int i = 0; i < _num_models; ++i)
...
void
ComputeMultiCPStress::postSolveQp(RankTwoTensor & cauchy_stress,
                                                    RankFourTensor & jacobian_mult)
{
...
void
ComputeMultiCPStress::solveStateVariables()
{
  ...
  do
  {
    ...
    {
      if (_print_convergence_message)
        mooseWarning("ComputeMultiCPStress: State variables (or the system "
  ...
  if (iteration == _maxiterg)
  {
    if (_print_convergence_message)
      mooseWarning(
          "ComputeMultiCPStress: Hardness Integration error. Reached the "
...
void
ComputeMultiCPStress::solveStress()
{
  ...
  {
    if (_print_convergence_message)
      mooseWarning("ComputeMultiCPStress: the slip increment exceeds tolerance "
                   "at element ",
                   _current_elem->id(),
                   " and Gauss point ",
                   _qp);
  ...
  while (rnorm > _rtol * rnorm0 && rnorm > _abs_tol && iteration < _maxiter)
  {
    ...
    {
      if (_print_convergence_message)
        mooseWarning("ComputeMultiCPStress: Failed with line search");
  ...
  {
    if (_print_convergence_message)
      mooseWarning("ComputeMultiCPStress: Stress Integration error rmax = ",
                   rnorm,
...
void
ComputeMultiCPStress::calculateResidualAndJacobian()
{
  calculateResidual();
  if (_convergence_failed)
    return;
...
void
ComputeMultiCPStress::calculateResidual()
{
...
void
ComputeMultiCPStress::calculateJacobian()
{
...
void
ComputeMultiCPStress::calcTangentModuli(RankFourTensor & jacobian_mult)
{
...
void
ComputeMultiCPStress::elastoPlasticTangentModuli(RankFourTensor & jacobian_mult)
{
...
void
ComputeMultiCPStress::elasticTangentModuli(RankFourTensor & jacobian_mult)
{
...
bool
ComputeMultiCPStress::lineSearchUpdate(const Real & rnorm_prev,
...
void
ComputeMultiCPStress::calculateEigenstrainDeformationGrad()
{
  _inverse_eigenstrain_deformation_grad.zero();
  _inverse_eigenstrain_deformation_grad.addIa(1.0);
