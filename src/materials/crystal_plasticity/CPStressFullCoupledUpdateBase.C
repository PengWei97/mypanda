...
InputParameters
CPStressFullCoupledUpdateBase::validParams()
{
  ...
  params.addParam<Real>(
      "coeff_heat_dissip",
      1.0,
      "Coefficient for heat dissipation in the constitutive model, default value is 1.0");
  params.addParam<Real>(
...
CPStressFullCoupledUpdateBase::CPStressFullCoupledUpdateBase(
  ...
  : Material(parameters),
    ...
    _coeff_heat_dissip(getParam<Real>("coeff_heat_dissip")),
    ...
    _slip_resistance_gr(_op_num),
    _slip_resistance_gr_old(_op_num),
    _slip_shear_strain_gr(_op_num),
    _slip_shear_strain_gr_old(_op_num),
    ...
    _print_convergence_message(getParam<bool>("print_state_variable_convergence_error_messages"))
{
  ...
  {
    _slip_resistance_gr[op_index] = &declareProperty<std::vector<Real>>(_base_name + "slip_resistance_gr" + std::to_string(op_index));
    _slip_resistance_gr_old[op_index] = &getMaterialPropertyOld<std::vector<Real>>(_base_name + "slip_resistance_gr" + std::to_string(op_index));

    _slip_shear_strain_gr[op_index] = &declareProperty<std::vector<Real>>(_base_name + "slip_shear_strain_gr" + std::to_string(op_index));
    _slip_shear_strain_gr_old[op_index] = &getMaterialPropertyOld<std::vector<Real>>(_base_name + "slip_shear_strain_gr" + std::to_string(op_index));
  }
}

void
CPStressFullCoupledUpdateBase::initQpStatefulProperties()
{
  ...
  // for (MooseIndex(_op_num) op_index = 0; op_index < _op_num; ++op_index)
  // {
  //   (*_slip_resistance_gr[op_index])[_qp].resize(_number_slip_systems);
  //   (*_slip_shear_strain_gr[op_index])[_qp].resize(_number_slip_systems);
  // }
...
void
CPStressFullCoupledUpdateBase::setMaterialVectorSize()
{
  _tau[_qp].resize(_number_slip_systems);
  _flow_direction[_qp].resize(_number_slip_systems);
  for (const auto i : make_range(_number_slip_systems))
...
void
CPStressFullCoupledUpdateBase::resetQpStatefulProperties(const unsigned int & op_index)
{
  _tau[_qp].resize(_number_slip_systems);
  _flow_direction[_qp].resize(_number_slip_systems);
  for (const auto i : make_range(_number_slip_systems))
...
void
CPStressFullCoupledUpdateBase::getSlipSystems()
{
  ...
  if (orthonormal_error)
    mooseError("CPStressFullCoupledUpdateBase Error: The slip system file contains a slip "
               "direction and plane normal pair that are not orthonormal in the Cartesian "
               "coordinate system.");
}

void
CPStressFullCoupledUpdateBase::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError("CPStressFullCoupledUpdateBase Error: The specified unit cell dimensions are "
               "not consistent with expectations for "
               "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError("CPStressFullCoupledUpdateBase Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CPStressFullCoupledUpdateBase Error: The specified HCP basal plane Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError("CPStressFullCoupledUpdateBase Error: The specified HCP slip direction "
...
void
CPStressFullCoupledUpdateBase::sortCrossSlipFamilies()
{
...
unsigned int
CPStressFullCoupledUpdateBase::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void
CPStressFullCoupledUpdateBase::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CPStressFullCoupledUpdateBase::calculateSchmidTensor(
...
void
CPStressFullCoupledUpdateBase::calculateShearStress(
...
void
CPStressFullCoupledUpdateBase::calculateTotalPlasticDeformationGradientDerivative(
...
void
CPStressFullCoupledUpdateBase::calculateEquivalentSlipIncrement(
...
void
CPStressFullCoupledUpdateBase::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CPStressFullCoupledUpdateBase::setActiveOpIndex(const unsigned int & active_op_index)
{
  _active_op_index = active_op_index;
}

void
CPStressFullCoupledUpdateBase::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CPStressFullCoupledUpdateBase::isConstitutiveStateVariableConverged(
...
void
CPStressFullCoupledUpdateBase::calculatePlasticityEnergy(
  Real & plastic_energy_grX,
  const unsigned int & op_index)
{
  plastic_energy_grX = 0.0;
  for (const auto i : make_range(_number_slip_systems))
  {
    auto & gamma_alpha = (*_slip_shear_strain_gr[op_index])[_qp][i];
    for (const auto j : make_range(_number_slip_systems))
    {
      auto & gamma_beta = (*_slip_shear_strain_gr[op_index])[_qp][j];
      ...
      if (iplane == jplane) // self vs. latent hardening
        plastic_energy_grX +=
            std::abs(gamma_alpha * gamma_beta); // q_{ab} = 1.0 for self hardening
      else
        plastic_energy_grX +=
            std::abs(gamma_alpha * gamma_beta) * 1.4; // latent hardenign
    }
  }

  plastic_energy_grX = _coeff_heat_dissip * std::pow(plastic_energy_grX, 3.0/4.0);
}