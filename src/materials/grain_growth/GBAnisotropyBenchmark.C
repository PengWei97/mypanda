...
#include "GBAnisotropyBenchmark.h"
#include "MooseMesh.h"

#include <fstream>

registerMooseObject("mypandaApp", GBAnisotropyBenchmark);

InputParameters
GBAnisotropyBenchmark::validParams()
{
...
GBAnisotropyBenchmark::GBAnisotropyBenchmark(const InputParameters & parameters)
  : Material(parameters),
    _mesh_dimension(_mesh.dimension()),
    _wGB(getParam<Real>("wGB")),
    _length_scale(getParam<Real>("length_scale")),
    _time_scale(getParam<Real>("time_scale")),
    _kb(getParam<Real>("kb")),      // Boltzmann constant in eV/K
    _JtoeV(getParam<Real>("JtoeV")), // Joule to eV conversion
...
{
  readGBAnisotropyData();

  computeGBAnisotropyParameters();
}

void 
GBAnisotropyBenchmark::readGBAnisotropyData()
{
  // reshape vectors
...
void
GBAnisotropyBenchmark::computeGBAnisotropyParameters()
{
  Real sigma_init;
  Real g2 = 0.0;
  Real f_interf = 0.0;
...
void
GBAnisotropyBenchmark::computeQpProperties()
{
  Real sum_kappa = 0.0;
  Real sum_gamma = 0.0;
  Real sum_L = 0.0;
