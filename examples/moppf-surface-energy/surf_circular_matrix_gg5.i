...
[Materials]
  [./free_energy_solid]
    ...
    expression = 'A0*(cv^4/4.0 - cv^2/2.0 + gr0^4/4.0 - gr0^2/2.0 + gr1^4/4.0 - gr1^2/2.0 + beta_s*cv^2*(gr0^2 + gr1^2) + beta_gb*gr0^2*gr1^2) + 1/4'
    derivative_order = 2
    enable_jit = true
...
[Outputs]
  [./my_exodus]
    ...
    type = Nemesis
    time_step_interval = 5 # The interval at which time steps are output
